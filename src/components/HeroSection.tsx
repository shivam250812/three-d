...
//         <motion.h2 className="text-5xl md:text-7xl font-bold leading-tight tracking-tight max-w-4xl mx-auto" initial={{opacity:0,y:50}} whileInView={{opacity:1,y:0}} transition={{duration:0.8, ease: "easeOut"}} viewport={{once:false}}>
...
const ModelWithManualLoad = () => {
  ...
  useEffect(() => {
    const loadModel = async () => {
      try {
        ...
        // Check what we're getting first
        const response = await fetch('/models/model.glb');
        ...
        // Check if it's actually a GLB file by looking at the first few bytes
        const view = new Uint8Array(arrayBuffer);
        const magic = new TextDecoder().decode(view.slice(0, 4));
        console.log('File magic number:', magic);
        
        if (magic !== 'glTF') {
          // It's not a GLB file, let's see what we got
          const textContent = new TextDecoder().decode(view.slice(0, 200));
          console.log('File content (first 200 chars):', textContent);
          throw new Error(`Invalid GLB file. Magic number: ${magic}, Content starts with: ${textContent}`);
        }

        // If we get here, it should be a valid GLB file
  ...
  if (error) {
    console.log('Rendering fallback cube due to error:', error);
    return (
      <mesh position={[0, -2.5, 0]} scale={1.5}>
        <boxGeometry args={[2, 2, 2]} />
        <meshStandardMaterial color="white" metalness={1.0} roughness={0.45} />
  ...
  return <primitive object={model} scale={1.5} position={[0, -2.5, 0]} />;
};
...
// Remove the preload for now to avoid caching issues
// useGLTF.preload('/models/model.glb');
